...
async function FinanceDashboardView() {
...
async function SupportDashboardView() {
  const session = await getSession();

  let urgentCount = 0, highCount = 0, mediumCount = 0, lowCount = 0;
  let pendingReports = 0;
  let myTickets: Array<import("@/types/api").Ticket> = [];
  let auditEntries: AuditEntry[] = [];

  await Promise.allSettled([
    serverGet<PaginatedResponse<import("@/types/api").Ticket>>("/admin/tickets?status=OPEN&priority=URGENT&limit=1", 0)
      .then((r) => { urgentCount = r.total; }),
    serverGet<PaginatedResponse<import("@/types/api").Ticket>>("/admin/tickets?status=OPEN&priority=HIGH&limit=1", 0)
      .then((r) => { highCount = r.total; }),
    serverGet<PaginatedResponse<import("@/types/api").Ticket>>("/admin/tickets?status=OPEN&priority=MEDIUM&limit=1", 0)
      .then((r) => { mediumCount = r.total; }),
    serverGet<PaginatedResponse<import("@/types/api").Ticket>>("/admin/tickets?status=OPEN&priority=LOW&limit=1", 0)
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Mes tickets assignés" value={myTickets.length}
          ...
          href={session?.userId ? `/admin/tickets?assigneeId=${session.userId}` : "/admin/tickets"} />
      </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-5 space-y-4">
          ...
          {totalOpen === 0 ? (
            ...
            <div className="space-y-3">
              {priorities.map(({ label, count, color, text, badge }) => (
      ...
      <div className="bg-white rounded-2xl border border-border overflow-hidden">
        ...
        {auditEntries.length === 0 ? (
          <div className="flex items-center justify-center py-10 text-sm text-text-disabled">Aucune activité récente</div>
        ) : (
          <ul className="divide-y divide-border">
            {auditEntries.map((entry) => (
              <li key={entry.id} className="px-5 py-3 flex items-start gap-3">
...
async function AccountManagerDashboardView() {
...
export default async function AdminDashboardPage() {
  ...
  if (isFinanceOnly)         return <FinanceDashboardView />;
  if (isSupportOnly)         return <SupportDashboardView />;
  if (isAccountManagerOnly)  return <AccountManagerDashboardView />;
